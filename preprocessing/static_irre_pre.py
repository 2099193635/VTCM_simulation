...
def plot_deviation_curves(
	...
	参数:
	- mileage_start / mileage_end: 实际里程区间（单位与 `里程` 列一致，通常 km）。
	  两者均填写时优先使用实际里程范围。
	...
	# Each subplot shows measured, design, deviation, and consistency residual.
	metrics = [
		("Left vertical", "实测左高低", "设计左高低", "左高低偏差"),
		("Right vertical", "实测右高低", "设计右高低", "右高低偏差"),
		("Left alignment", "实测左轨向", "设计左轨向", "左轨向偏差"),
		("Right alignment", "实测右轨向", "设计右轨向", "右轨向偏差"),
		("Gauge", "实测轨距", "设计轨距", "轨距偏差"),
		("Superelevation", "实测水平", "设计水平", "水平偏差"),
	]
	...
	use_mileage_range = mileage_start is not None and mileage_end is not None
	if use_mileage_range:
		m0 = float(mileage_start)
		m1 = float(mileage_end)
		if m1 < m0:
			m0, m1 = m1, m0
		m0 = max(x_min, m0)
		m1 = min(x_max, m1)
		if m1 <= m0:
			raise ValueError(
				f"选定里程范围 [{mileage_start}, {mileage_end}] 与数据范围 "
				f"[{x_min:.6f}, {x_max:.6f}] 无有效交集。"
			)
		range_desc = f"mileage {m0:.6f} - {m1:.6f} km"
	else:
		# 里程百分比裁剪
	...
	for i, (name, c_meas, c_design, c_dev) in enumerate(available[:6]):
		...
		# 强化线型区分，便于近距离观察
		ax.plot(x, meas, linewidth=1.4, color="#1f77b4", linestyle="-", label="Measured")
		ax.plot(x, design, linewidth=1.3, color="#2ca02c", linestyle=":", label="Design")
		ax.plot(x, dev, linewidth=1.3, color="#ff7f0e", linestyle="-.", label="Deviation")
		ax.plot(x, resid, linewidth=1.2, color="#d62728", linestyle="--", label="Residual: deviation - (measured - design)")
	...
	fig.suptitle(
		f"Measured, design, deviation, and residual curves ({range_desc})",
		fontsize=14,
	)
...
def process_static_file(
	raw_file: str | Path,
	processed_dir: str | Path,
	plot_dir: str | Path,
	file_prefix: Optional[str] = None,
	save_tidy_csv: bool = True,
	plot_pct_start: float = 0.0,
	plot_pct_end: float = 1.0,
	plot_mileage_start: Optional[float] = None,
	plot_mileage_end: Optional[float] = None,
) -> dict:
	"""
	处理单个静检 .jdt 文件：
	1) 读取与统一里程（绝对里程）
	2) 绘制偏差检测图（保存到指定绘图目录）
	3) 导出 irregularity 外部导入结构（绝对里程）
	4) 按需保存 tidy CSV（默认保存）
	"""
	...
	stem = file_prefix or raw_file.stem

	data = read_jdt(raw_file)
	tidy = tidy_jdt_with_single_mileage(data)

	# 静检绘图固定保存到原始文件/绘图检测
	plot_file = plot_dir / f"{stem}.deviation_subplots.png"
	plot_deviation_curves(
		tidy,
		plot_file,
	...
	tidy_file = None
	if save_tidy_csv:
		tidy_file = save_table(tidy, processed_dir / f"{stem}.tidy.csv", encoding="utf-8-sig")

	# 导出外部导入文件，里程保留绝对里程
	ext_dir = processed_dir / f"{stem}.external"
	...
	return {
		...
		"tidy_file": str(tidy_file) if tidy_file else None,
		"external_files": ext_paths,
		"tidy_df": tidy,
	}


def merge_tidy_static_frames(tidy_frames: list[pd.DataFrame], mileage_col: str = "里程") -> pd.DataFrame:
	"""拼合多个静检 tidy 表：按绝对里程排序，并对同里程点做聚合去重。"""
	if not tidy_frames:
		return pd.DataFrame(columns=[mileage_col])

	merged = pd.concat(tidy_frames, axis=0, ignore_index=True, sort=False)
	if mileage_col not in merged.columns:
		raise ValueError("拼合静检数据失败：缺少里程列。")

	merged[mileage_col] = pd.to_numeric(merged[mileage_col], errors="coerce")
	merged = merged[np.isfinite(merged[mileage_col].to_numpy(dtype=float))].copy()

	# 同里程去重：数值列取均值，非数值列取首个有效值
	numeric_cols = [c for c in merged.columns if c == mileage_col or pd.api.types.is_numeric_dtype(merged[c])]
	agg = {}
	for c in merged.columns:
		if c == mileage_col:
			continue
		agg[c] = "mean" if c in numeric_cols else "first"

	merged = merged.groupby(mileage_col, as_index=False).agg(agg)
	merged = merged.sort_values(mileage_col, kind="mergesort").reset_index(drop=True)
	return merged
...
def process_static_files(
	...
	1) 分别读取并整理为统一里程
	2) 按绝对里程拼合与去重
	3) 绘图、保存 tidy、导出 external 结构（绝对里程）
	"""
	if not raw_files:
		raise ValueError("raw_files 为空，未找到可拼合的静检 .jdt 文件。")
	...
	for rf in raw_files:
		...
		if not p.exists():
			continue
		raw_df = read_jdt(p)
		tidy = tidy_jdt_with_single_mileage(raw_df)
		tidy_list.append(tidy)
		raw_list.append(str(p))

	if not tidy_list:
		raise ValueError("未读取到有效静检文件，无法拼合。")

	tidy_merged = merge_tidy_static_frames(tidy_list, mileage_col="里程")

	plot_file = plot_dir / f"{file_prefix}.deviation_subplots.png"
	plot_deviation_curves(
		tidy_merged,
		plot_file,
		mileage_pct_start=plot_pct_start,
	...
	tidy_file = None
	if save_tidy_csv:
		tidy_file = save_table(tidy_merged, processed_dir / f"{file_prefix}.tidy.csv", encoding="utf-8-sig")

	ext_dir = processed_dir / f"{file_prefix}.external"
	ext_paths = export_for_irregularity_external_import(
		tidy_merged,
		ext_dir,
...
if __name__ == "__main__":
	sample = Path(r"preprocessing\静检数据\呼局\20210416\原始文件\静检上行20210416-276-278.jdt")
	# 绘图实际里程范围（km，可调）；填 None 时使用百分比区间
	plot_mileage_start = None
	plot_mileage_end = None
	# 里程百分比绘图区间（可调，未填写实际里程范围时生效）
	plot_pct_start = 0.45
	plot_pct_end = 0.55
	...
	if sample.exists():
		...
		result = process_static_file(
			raw_file=sample,
			processed_dir=Path(r"preprocessing\静检数据\呼局\20210416\处理后"),
			plot_dir=Path(r"preprocessing\静检数据\呼局\20210416\原始文件\绘图检测"),
			file_prefix=sample.stem,
			save_tidy_csv=True,
			plot_pct_start=plot_pct_start,
			plot_pct_end=plot_pct_end,
			plot_mileage_start=plot_mileage_start,
			plot_mileage_end=plot_mileage_end,
		)
		print("Processed static file done:")
		print({k: v for k, v in result.items() if k != "tidy_df"})
	else:
